...
from .config import ROOT, exp_root  # noqa: F401  (ROOT re-exported for notebook use)


def _win(window):
    """Resolve a window: a name from the DOMAIN's registry, an explicit tuple, or None.

    ⚠️ THESE USED TO BE A FLAT MODULE-LEVEL DICT spanning every domain, and before that
    they lived in a notebook setup cell — so any script that touched them died on KeyError
    unless that cell had been executed first. The flat dict worked only because the keys
    happened not to collide, which is luck, not design. Named map windows are per-domain
    geography and belong in ``Domain.map_windows`` like everything else geographic.
    """
    if window is None or not isinstance(window, str):
        return window
    wins = _domain.active().map_windows
    if window not in wins:
        raise KeyError(
            f"domain {_domain.active().name!r} has no map window {window!r}; "
            f"known: {sorted(wins)}. Add it to Domain.map_windows, not here."
        )
    return wins[window]
