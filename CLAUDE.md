...
| `v4` | v3 + Delaware Bay to Trenton (forced at the mouth) + Raritan to its head of tide + the Arthur Kill shore **and Newark Bay** (09-25); **far banks (DE/PA/Staten Island) COMPUTED to +10 m, not walled**; Track C coarse shelf | **IN DESIGN** (acquisition-only) — the RING is being redrawn fresh as straight lines (09-25); checked crossings in `data/v4_design/v4_crossings.geojson`, audit any ring with `scripts/audit_region_v4.py --ring <file>`; rules, build items and data gaps in STATUS PICK UP; the design's own gate is a waves-off Sandy +0/+2/+3 m overflow test |
