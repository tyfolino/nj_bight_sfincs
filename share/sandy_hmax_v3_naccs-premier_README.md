...
`sandy_hmax_v3_naccs-premier_EPSG32618.tif`
...
Model run: domain `v3`, experiment `naccs-premier`, exported 2026-09-23.
...
```python
import rioxarray
da = rioxarray.open_rasterio("sandy_hmax_v3_naccs-premier_EPSG32618.tif", masked=True).squeeze()
...
Wet cells: 26,634,393 (1040 km², 4.30% of the grid rectangle);
depth range 0.05 to 11.98 m. At or above the 0.15 m threshold:
24,535,016 cells (958 km²), median depth 1.22 m, 99th percentile
2.88 m.
...
- **High-water marks:** 94 USGS marks scored; bias **-0.14 m**, RMSE 0.33 m (median estimator, 50 m search radius — the bias depends on both, so always quote them with it).
