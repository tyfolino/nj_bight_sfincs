...
"""How much of CORA's own wave height does a SnapWave arm deliver to the −9 m shelf?
    ...
    NJ_DOMAIN=v3 python scripts/wave_shelf_reference.py experiments/v3/<arm> \
        [--premier experiments/v3/naccs-premier] [--times ...]
...
mesh) as the reference instead, per site along a shore-normal line:
  ...
  cora_old10m      CORA hs at the PREMIER's boundary cells (the −10 m line)
  ...
  premier column   the same ratio on the premier, for the paired view
...
DEFAULT_TIMES = ("2012-10-28T12:00", "2012-10-29T00:00", "2012-10-29T12:00",
                 "2012-10-30T00:00")
...
def main() -> None:
    ...
    ap.add_argument("--premier", type=Path, default=None,
                    help="the −10 m-boundary reference run (default exp_root()/naccs-premier)")
    ap.add_argument("--times", nargs="*", default=list(DEFAULT_TIMES))
    args = ap.parse_args()
    dom = _domain.active()
    prem = args.premier or (args.run.parent / "naccs-premier")
    ...
    pm = xr.open_dataset(prem / "sfincs_map.nc", **kw)
    swm, pswm = mp["snapwavemsk"].values, pm["snapwavemsk"].values
    ...
    for name, (lon, lat, az) in SITES.items():
        ...
        if not len(newb):  # a stepped line can miss the strip: nearest boundary cell to its far end
            far = np.argmax(np.where(sel, s, -1))
            newb = np.array([np.argmin(np.hypot(fx - fx[far], fy - fy[far]) + 1e9 * (swm != 2))])
        for tt in args.times:
            ti = np.datetime64(tt)
            r = dict(site=name, time=tt[5:16], newbnd_km=round(float(s[newb].mean()) / 1e3, 1),
                     newbnd_z=round(float(np.nanmedian(z[newb])), 0))
            if ti > tlast:
                rows.append(r); continue
            hh = mp["hm0"].isel(time=int(np.argmin(np.abs(mp.time.values - ti)))).values
            ph = pm["hm0"].isel(time=int(np.argmin(np.abs(pm.time.values - ti)))).values
            cn, co = cora_at(newb, ti), cora_at(oldb, ti)
            r.update(cora_newbnd=cn, cora_old10m=co, SWAN_keeps=co / cn,
                     SnapWave_keeps=float(np.nanmean(hh[oldb])) / cn,
                     shelf9m_over_cora10m=float(np.nanmean(hh[shelf])) / co,
                     premier_shelf9m_over_cora10m=float(np.nanmean(ph[shelf])) / co)
            rows.append(r)
    pd.set_option("display.width", 220)
    df = pd.DataFrame(rows).round(2)
    print(f"arm {args.run.name} vs CORA (map ends {str(tlast)[:16]}); criterion: "
