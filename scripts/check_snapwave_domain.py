...
def main() -> None:
    ...
    ap.add_argument("--mesh", type=Path, default=None, help="sfincs.nc (default: frozen mesh)")
    ...
    if info["n_sfincs_outside_band"]:
        print("   🔴 SFINCS-active cells lie EAST of the band limit — move that step seaward.")

    # Demotion north of the open coast, as add_waves does it.
    demote = (swm == 2) & (fy >= (dom.open_coast_max_y or np.inf))
    swm2 = swm.copy()
    swm2[demote] = 1
    ...
    for lo, hi, M in steps.steps:
        ...
        for lab, sel in (("N-S column", col), ("E-W step/bottom", stp)):
            if sel.any():
                rows.append(dict(rows=f"{lo}-{hi}", M=M, part=lab, n=int(sel.sum()),
                                 z_min=np.nanmin(z[sel]), z_p5=np.nanpercentile(z[sel], 5),
                                 z_med=np.nanmedian(z[sel]), z_p95=np.nanpercentile(z[sel], 95),
                                 z_max=np.nanmax(z[sel]),
                                 levels=dict(zip(*[a.tolist() for a in np.unique(lev[sel], return_counts=True)])),
    ...
    print(f"   ring {rr['n_ring']}  one-neighbour {rr['n_one']}  two-plus {rr['n_two_plus']} "
    ...
    if args.waves is not None:
        ...
        d = np.array([np.min(np.hypot((slon - lo) * np.cos(np.deg2rad(la)), slat - la)) * 111_000
                      for lo, la in zip(plon, plat)])
        j = np.array([int(np.argmin(np.hypot((slon - lo) * np.cos(np.deg2rad(la)), slat - la)))
                      for lo, la in zip(plon, plat)])
        print(f"   nearest {args.waves.name} node: median {np.median(d) / 1e3:.2f} km, "
              f"max {d.max() / 1e3:.2f} km (limit 5 km)"
              + (f"; node depth {np.min(dep[j]):.1f}..{np.max(dep[j]):.1f} m" if dep is not None else ""))
        if d.max() > 5000:
            print("   🔴 a support point has no wave node within 5 km — add_waves will refuse.")
    ...
    if args.geojson is not None:
        ...
        line = gpd.GeoDataFrame({"name": [steps.name]}, geometry=[LineString(poly)], crs=dom.epsg)
        sp = gpd.GeoDataFrame({"k": range(len(pts))}, geometry=[Point(a, b) for a, b in pts], crs=dom.epsg)
        args.geojson.parent.mkdir(parents=True, exist_ok=True)
        pd.concat([cells.assign(kind="boundary_cell"), line.assign(kind="polyline"),
                   sp.assign(kind="support_point")]).to_file(args.geojson, driver="GeoJSON")
