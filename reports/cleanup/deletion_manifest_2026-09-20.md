...
session; sections B–C EXECUTED 2026-09-21 the same way (ticked = done; unticked = the user said no, or the
claim below was wrong and is corrected in place).** Nothing in this file is pending. Precedent:
...
Retired-arm / closed-sweep tooling (⚠️ 09-21: "zero code references" was WRONG for eight of these — checked one by one before removal; comment mentions of removed scripts now say `retired 2026-09-21, in git history`):
- [x] `scripts/build_stockdon_boundary.py` — built `BRACKET+setup-stockdon` (retired 09-18)
- [x] `scripts/build_naccs_stwave_waves.py` — built `wave-stwave` (retired 09-18)
- [x] `scripts/measure_rain_share.py` — `diag-premier-norain` read (retired; FINDINGS §39 holds it); its `reports/rain/*.csv` go with it
- [x] `scripts/sweep_bed_dams.py` + `scripts/plot_bed_dam_candidates.py` — the bridge-as-dam sweep, closed 08-26 ("no carve"); `reports/bed_dams_v3.csv` + `reports/figures/bed_dam*.png` go with them
- [ ] `scripts/stamp_metrics_epoch.py` — one-off 09-11 migration, done — 🔴 KEPT 09-21: `tests/test_engine_epoch.py` loads it.
- [x] `scripts/town_floodmap_diff.py` + `scripts/motf_csi_buildings_masked.py` — the 09-08 buildings read, closed (`experiments/v3/motf_csi_buildings_masked.csv` stays as the record)
- [x] `scripts/wave_boundary_ring.py` — the 09-08 "boundary not transmitting" diagnosis; the defect is fixed and the ring is in FINDINGS
- [ ] `scripts/score_bracket.py` + `scripts/score_v2_barnegat.py` — v2 Manahawkin bracket / archived v2 runs (⚠️ `v2_barnegat` still has 32 code references, mostly `domain.py` comments — check `grep -rn v2_barnegat nj_sfincs` before removing anything BUT these two scripts) — 🔴 BOTH KEPT 09-21: `premier.py:427`'s refusal message tells the reader to run `score_bracket.py`; v2 is still a registered score-only domain and its `metrics.csv` is kept.
- [ ] `scripts/download_era5_waves_cds.py` — ERA5 waves are inadmissible (FINDINGS §21); its output is not used — 🔴 KEPT 09-21, the claim was wrong: `era5_waves_nj` is still `WaveConfig.wave_geodataset`'s default and `data_catalog.yml` names this script as its download (provenance).

v1.5-only build steps (v1.5 is FROZEN; the mesh + data stay for the registry):
- [ ] `scripts/build_refinement_v1_5.py`, `scripts/validate_region_v1_5.py` — 🔴 SPLIT 09-21: `build_refinement_v1_5.py` KEPT (the only refinement-recipe builder in the repo, carries the named bands of CLAUDE.md §5's first trap; v4 ports from it); `validate_region_v1_5.py` REMOVED.
- [ ] `scripts/build_coned_sw_raritan.py`, `scripts/plot_ward_point_bed.py` — the Ward Point / CoNED tier decision (08-15), settled — 🔴 SPLIT 09-21: `build_coned_sw_raritan.py` KEPT (built the `coned_sw_raritan` tier in the frozen v1.5 bed = provenance; cited by `config.py`); `plot_ward_point_bed.py` REMOVED.
- [x] `scripts/diagnose_keansburg.py` — the Keansburg overshoot → the weir (FINDINGS §38); `reports/keansburg/` (9 tracked files) goes with it
- [ ] `scripts/sweep_cudem_flatfill.py`, `scripts/audit_paved_channels.py` — v1.5 bed workstreams; `reports/coned/phantom_water_patches.json` goes with the first — KEPT 09-21 (user): cited by `config.py`, `accept_domain.py`, `download_ehydro_nj.py` as the reason for kept tiers.
- [x] `scripts/rebuild_subgrid_h.py` — superseded by `scripts/rebuild_subgrid.py` (which has the `--overlay` path CLAUDE.md §5 requires)
...
- [x] `notebooks/v3/sandy-v3-viz-2026-08-29.ipynb` (12 M) — renders the three 08-27 v3 arms that were **VOIDED** on 08-31 (the dropped refinement bands, CLAUDE.md §5). A rendered notebook of void runs is the textbook stale artifact.
- [x] `notebooks/v3/sandy-v3-winddir-fix-2026-09-13.ipynb` (21 M) — the bug-era vs fixed-engine comparison; FINDINGS §43 and the 09-14 notebook carry the result
- [ ] `notebooks/v1_5_raritan/` (2 files, 6 M) — the frozen predecessor's viz — SPLIT 09-21: only the undated `sandy-v1_5-viz.ipynb` REMOVED (a 20-cell older subset of the dated one); `sandy-v1_5-viz-2026-08-21.ipynb` KEPT with the v1.5 record.
- [x] `reports/snapwave_parameters_naccs-premier.docx` (13 K) — superseded by `reports/snapwave_parameters.html` + `docs/snapwave_parameters.md`
...
## Also done 2026-09-21 (plan Phase 7 leftovers, user-approved)

- [x] `/scratch/tpj8/engine_gate/{G5_v233,G5_main_default,G5_main_gammax2,D4_bds60}` (32.5 G apparent; compare JSONs, `repro/`, `repro_ig/` kept)
- [x] `data/probe_mesh_v1_5_fix4` (17 M, untracked probe)
- `scripts/download_era5_overwater_vars.py` (on the PLAN's `git rm` list, not this one) KEPT: it made `data/era5/`, the wind the model runs on.
- Not touched: `reports/figures/bed_dam*.png` (gitignored), `~/fish-4.7.1{,.tar}`, `~/.claude.json.tmp.*` (the user's).
- After: 174 tests OK (7 skipped); scratch 154.3 → 59.2 G over the day.
